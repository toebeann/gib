      ...
      (async () => {
        ...
        if (shouldAddShortcut && !switchSupported) {
          ...
          if (!filenames.includes(bepinexScriptName)) {
            throw "Downloaded BepInEx pack appears invalid";
          }
  ...
  if (wantsCheckPath) {
    ...
    if (!commandExists) {
      ...
      const attemptAddCommandToPath = async () => {
        ...
        if (
          !SHELL ||
          !(["fish", "zsh", "bash"] as const).includes(basename(SHELL))
        ) {
          ...
          try {
            const commandsToAppend = await configCommandsNeeded(
              config,
              commands,
            );
